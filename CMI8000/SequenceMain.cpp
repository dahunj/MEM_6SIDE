...
CSequenceMain::CSequenceMain()
{
	...
	gData.nTrayCntNG[PORT1][2] = 1;
	gData.nTrayCntNG[PORT1][3] = 1;
	gData.nTrayCntNG[PORT1][0] = gData.nTrayCntNG[PORT1][2] + 1;
	gData.nTrayCntNG[PORT1][1] = gData.nTrayCntNG[PORT1][3] + 1;

	gData.nTrayCntNG[PORT2][2] = 1;
	gData.nTrayCntNG[PORT2][3] = 1;
...
void CSequenceMain::Job_LotEnd(int nPortNo, int nGTNo)
{
	...
	if(nCMCnt_SE <= 0) nCMCnt_SE = 1;
	gLot.dTactTime_StoETime = (dwTime_StoE / 1000.0) / (nCMCnt_SE);
	...
	if(nCMCnt_Unload <= 0) nCMCnt_Unload = 1;
...
BOOL CSequenceMain::LoadTray_Run()
{
	...
	static DWORD dwStartTick = 0, dwEndTick = 0;
	...
	case 18:	// Check Lot Ready
		dwEndTick = GetTickCount() - dwStartTick;
		...
		else if(dwEndTick > 25000)
		{
			m_nLoadTrayCase = 19; m_tLoadTrayLoop.Set_LoopTime(5000);
		}
		break;
	case 19:
		dwStartTick = GetTickCount();
		g_objInspector.Set_LotStart(INSPECTOR_ALL, gData.sLotID[nLtWorkPort-1], nLtWorkPort, m_pEquipData->sVisionProgVer, m_pEquipData->sVisionParaVer);
		gData.bVisionVerErr = FALSE;
		m_nLoadTrayCase = 18; m_tLoadTrayLoop.Set_LoopTime(20000);
		break;
...
BOOL CSequenceMain::LoadPicker_Run()
{
	...
	case 17:	// Position Check
		...
		{			
			...
			m_tLoadPickLoop.Takt_End(nTaktZone,17,TRUE);
		}
		break;
	}
...
BOOL CSequenceMain::VisionAngle_Run()
{
	...
	case 24:	// Angle Unit Position Check
			...
			g_objCommon.Check_Position(AX_ANGLE_UNIT_R, 0) && g_objCommon.Check_Position(AX_VISION_ANGLE_Z, 0)) 
		{
			...
			m_tVisAngleLoop.Takt_End(nTaktZone, 23, TRUE);
			//m_tVisAngleLoop.Takt_Start(nTaktZone, 24, "", TRUE);
		}
		break;
	}
...
BOOL CSequenceMain::AngleTray1_Run()
{
	...
	case 10:
		...
		{			
			int nCmNo = 0;
			for(int nTrayPosY = 0; nTrayPosY < 3; nTrayPosY++)
			{
				for(int nTrayPosX = 0; nTrayPosX < 4; nTrayPosX++)
				{
					if(gData.InfoAngleTray[0][nTrayPosY][nTrayPosX] > 0)
					{
						nCmNo = (gData.nTrayY - 1 - nTrayPosY) * gData.nTrayX +1 +nTrayPosX;
						g_objLogFile.Save_PositionLog(gData.nPNoAngleTray[0],gData.nTNoAngleTray[0], nCmNo, AX_ANGLE_STAGE1_Z, 2);
					}
				}
			}			
	...
	case 53:	// Check Position
		...
		{			
			...
			m_tAngleTray1Loop.Takt_End(nTaktZone, 53, TRUE);
		}
		break;
	}
...
BOOL CSequenceMain::AngleTray2_Run()
{
	...
	case 10:
		...
		{			
			...
			for(int nTrayPosY = 0; nTrayPosY < 3; nTrayPosY++)
			{
				for(int nTrayPosX = 0; nTrayPosX < 4; nTrayPosX++)
				{
					if(gData.InfoAngleTray[1][nTrayPosY][nTrayPosX] > 0)
					{
						nCmNo = (gData.nTrayY - 1 - nTrayPosY) * gData.nTrayX +1 +nTrayPosX;
						g_objLogFile.Save_PositionLog(gData.nPNoAngleTray[1],gData.nTNoAngleTray[1], nCmNo, AX_ANGLE_STAGE2_Z, 2);
					}
				}
			}
	...
	case 53:	// Check Position
		...
		{			
			...
			m_tAngleTray2Loop.Takt_End(nTaktZone, 53, TRUE);
		}
		break;
	}
...
BOOL CSequenceMain::Btm1Picker_Run()
{
	...
	case 1:		// Picker 
			...
			g_objCommon.Check_Position(AX_BTM1_PICKER_P1, 0) && g_objCommon.Check_Position(AX_BTM1_PICKER_P2, 0))
		{
			...
			m_tBtm1PickLoop.Takt_Start(nTaktZone, 1, TRUE);	m_tBtm1PickLoop.Takt_End(nTaktZone, 1);
	...
	case 3:		// Btm1 Picker Down
		if (g_objAJinAXL.Is_MoveDone(AX_BTM1_PICKER_X, dB1pX) ) {
			if ((nB1pWorkTray == 1 && g_objAJinAXL.Is_MoveDone(AX_ANGLE_STAGE1_Y, dB1pTrayY)) ||
				...
				g_objCommon.Move_Position(AX_BTM1_PICKER_Z, BTM1_PICKER_Z_TrayDown);	// Load Down
	...
	case 4:		// Btm1 Picker Grip Close
		if (g_objCommon.Check_Position(AX_BTM1_PICKER_Z, BTM1_PICKER_Z_TrayDown)) 
		{			
			if ((nB1pRow == 1 && g_objCommon.Get_Btm1PickerDown(5)) ||
				(nB1pRow == 0 && g_objCommon.Get_Btm1Picker2RowUp())) 
			{
				...
				for (int i = 0; i < gData.nBtmPickQt; i++) 
				{
					...
					g_objLogFile.Save_PositionLog(gData.nPNoAngleTray[nB1pWorkTray - 1], gData.nTNoAngleTray[nB1pWorkTray - 1], gData.nCNoBtm1Pick[nBtmX+i]
						, AX_BTM1_PICKER_Z, BTM1_PICKER_Z_TrayDown );
	...
	case 9:		// Set Load Complete
		if (g_objAJinAXL.Is_MoveDone(AX_BTM1_PICKER_X, dB1pX)  
			...
			&& g_objAJinAXL.Is_MoveDone(AX_BTM1_PICKER_X, dB1pX) &&
			g_objCommon.Check_Position(AX_BTM1_PICKER_P1, 0) && g_objCommon.Check_Position(AX_BTM1_PICKER_P2, 0) 
			)
		{			
	...
	case 29:		// Set Load Complete
		if (g_objAJinAXL.Is_MoveDone(AX_BTM1_PICKER_X, dB1pX)  && (g_objCommon.Check_Position(AX_BTM1_PICKER_Z, 0) || g_objCommon.Check_Position(AX_BTM1_PICKER_Z, 4) )
			&& g_objCommon.Check_Position(AX_BTM1_SHIFT_Y, 0)) 
		{
	...
	case 33:	// Btm1 Shift Y Move
		...
		break;
	case 34:	// Send Shift Complete
		if (g_objCommon.Check_Position(AX_BTM1_SHIFT_Y, 1) && g_objAJinAXL.Is_MoveDone(AX_BTM1_PICKER_X, dB1pX) && g_objAJinAXL.Is_Done(AX_BTM1_PICKER_Z)) {
			g_objInspector.Set_SMoveComplete(INSPECTOR_PC1, "B1AG");
			m_nBtm1PickCase = 30; m_tBtm1PickLoop.Set_LoopTime(30000);	// 90초
		}
		break;
	...
	case 35:	// X Move to Pitch & Inspection End Check
		...
		break;
	case 36:
		if (g_objCommon.Check_Position(AX_BTM1_SHIFT_Y, 0))
		{
		...
		break;
	case 40:
		if (g_objAJinAXL.Is_Done(AX_BTM1_PICKER_X) && g_objAJinAXL.Is_Done(AX_BTM1_SHIFT_Y))
		{
			if (gData.bReload[6]) {
	...
	case 41:
		if (g_objCommon.Check_Position(AX_BTM1_PICKER_X, 6) && g_objCommon.Check_Position(AX_BTM1_PICKER_Z, BTM1_PICKER_Z_BTM13D)
			&& g_objCommon.Check_Position(AX_BTM1_SHIFT_Y, 0)) 
		{
			int nBtmX = 4 * nB1pRow;
	...
	case 22:	// 안전 확인 후 Picker Down, Inspection Stage Vac On
		if (g_objCommon.Check_Position(AX_BTM1_PICKER_X, 5) 
			...
			if ((nB1pInspStageNo == 1 && m_pDX05->iInspectStage1Up && !m_pDX05->iInspectStage1Down) ||
				...
				g_objCommon.Set_Btm1PickerDown();
				g_objCommon.Move_Position(AX_BTM1_PICKER_Z, BTM1_PICKER_Z_Inspect);	//Unload Down
	...
	case 23:	// Btm1 Picker Vac Off
		if (g_objCommon.Check_Position(AX_BTM1_PICKER_Z, BTM1_PICKER_Z_Inspect) && g_objCommon.Get_Btm1PickerDown(0)) 
		{
			for (int i = 0; i < 8; i++) 
			{
				g_objLogFile.Save_PositionLog(gData.nPNoInspect[nB1pInspStageNo-1], gData.nTNoInspect[nB1pInspStageNo-1][0], gData.nCNoInspect[nB1pInspStageNo-1][i], AX_BTM1_PICKER_Z, BTM1_PICKER_Z_Inspect);
			}
...
BOOL CSequenceMain::Inspection1_Run()
{
	...
	case 3:		// CM Align Master/
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoInspect[0],gData.nTNoInspect[0][0], -1, AX_MODULE_ALIGN_Z, MODULE_ALIGN_Z_StopDown);
			
			m_pDY06->oCmAlignMasterOpen = FALSE; m_pDY06->oCmAlignMasterClose = TRUE;
			g_objAJinAXL.Write_Output(6);
			m_nInspect1Case = -1; m_tInspect1Loop.Set_LoopTime(5000);
	...
	case 14:	// Send Move Complete
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoInspect[0],gData.nTNoInspect[0][0], -1, AX_TOP1_MIRROR_Z, TOP1_MIRROR_Z_WorkDown);
				...
				if (g_objCommon.Check_Position(AX_INSPECT_STAGE3_X, 4) || (dCurPos >= dT2Pos && dCurPos <= dUnPos) ||
					...
					{						
						...
						if(nIs1ScanNo == 3 && m_nInspect2Case == 1 && g_objAJinAXL.Is_Done(AX_INSPECT_STAGE2_X)
							&& g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 0) && g_objCommon.Get_InfoInspectVacOn(2))
						{ 
	...
	case 50:	// Wait 회피 위치, Stage가 모두 Up 상태면 진행.
		...
		{			
			m_tInspect1Loop.Takt_Start(nTaktZone, 50);
			m_tInspect1Loop.Takt_End(nTaktZone,50);
			m_nInspect1Case++; m_tInspect1Loop.Set_LoopTime(10000);
		}
		return TRUE;
	...
	case 52:	// Position Check
		if (g_objCommon.Check_Position(AX_INSPECT_STAGE1_X, 0)) {
			...
			m_tInspect1Loop.Takt_Start(nTaktZone, 52);
	...
	case 70:	// Wait 회피 위치, Top1 Position 부터 진행한다.
		if (m_nInspect3Case < 80 && m_nInspect3Case > 7 && !g_objCommon.Check_Position(AX_INSPECT_STAGE3_X, 0))
		{
			...
			if (dCurPos >= dT1Pos)
			{	//위치 확인
				m_tInspect1Loop.Takt_End(nTaktZone,52);
				m_nInspect1Case++; m_tInspect1Loop.Set_LoopTime(10000);
			}
		}
		return TRUE;
	...
	case 72:	// Stage Up Check
		if (m_pDX05->iInspectStage1Up && !m_pDX05->iInspectStage1Down) {
			...
			m_tInspect1Loop.Takt_End(nTaktZone,71,TRUE);
		}
		break;
...
BOOL CSequenceMain::Inspection2_Run()
{
			...
			if ((m_nInspect1Case > 7 && m_nInspect1Case <= 31)|| bInspStart) {
				...
				if ((dCurPos >= dT1_4thPos && dCurPos <= dUnPos)|| bInspStart) {	//위치 확인
					if (g_objCommon.Check_Position(AX_INSPECT_STAGE2_X, 0) && g_objCommon.Get_InfoInspectVacOn(2) &&
						m_pDX05->iInspectStage2Up && !m_pDX05->iInspectStage2Down && m_pDX05->iInspectStage2Fwd 
						&& !m_pDX05->iInspectStage2Bwd && g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 0)) 
					{
	...
	case 3:		// CM Align Master/
		if (g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 1)) 
		{
			g_objLogFile.Save_PositionLog(gData.nPNoInspect[1],gData.nTNoInspect[1][0], -1, AX_MODULE_ALIGN_Z, MODULE_ALIGN_Z_StopDown);
			
			m_pDY06->oCmAlignMasterOpen = FALSE; m_pDY06->oCmAlignMasterClose = TRUE;
			g_objAJinAXL.Write_Output(6);
			m_nInspect2Case = -1; m_tInspect2Loop.Set_LoopTime(5000);
	...
	case 5:
		...
		break;
	case 6:		// Align Z Axis Move to Ready Up Position
		if (g_objCommon.Check_Position(AX_MODULE_ALIGN_Y, 0) && g_objCommon.Get_InfoInspectVacOn(2) &&
			m_pDX06->iCmAlignMasterOpen && !m_pDX06->iCmAlignMasterClose &&
			m_pDX06->iCmAlignSlaveOpen  && !m_pDX06->iCmAlignSlaveClose) 
		{			
	...
	case 14:	// Send Move Complete
		if (g_objCommon.Check_Position(AX_TOP1_MIRROR_Z, 1) && g_objAJinAXL.Is_Done(AX_VISION_TOP1_Z)) 
		{
			g_objLogFile.Save_PositionLog(gData.nPNoInspect[1],gData.nTNoInspect[1][0], -1, AX_TOP1_MIRROR_Z, TOP1_MIRROR_Z_WorkDown);
	...
	case 16:	// 다음 검사 위치 피치 이동
		if (g_objAJinAXL.Is_Done(AX_INSPECT_STAGE2_X) && g_objCommon.Check_Position(AX_VISION_TOP1_Z, 0) && g_objCommon.Check_Position(AX_TOP1_MIRROR_Z, 0)) {
			...
			if (nIs2ScanNo > gData.nScanCnt) {	// 모델별로 피치이동 횟수가 다르다.
				...
				if (g_objCommon.Check_Position(AX_INSPECT_STAGE1_X, 4) || (dCurPos >= dT2Pos && dCurPos <= dUnPos) ||
					...
					if (Select_Top1ScanPos(AUTO_INSPECTION2, nIs2ScanNo)) {
						...
						if(nIs2ScanNo == 3 && m_nInspect3Case == 1 && g_objAJinAXL.Is_Done(AX_INSPECT_STAGE3_X)
							&& g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 0) && g_objCommon.Get_InfoInspectVacOn(3))
						{ 
							// Top1 마지막 피치 이동시 뒤에 있는 스테이지도 동시 어라인 진입 
				...
				{
					m_tInspect2Loop.Set_LoopTime(10000); 
					return TRUE;
				}
			}
		}
	...
	case 52:	// Position Check
		if (g_objCommon.Check_Position(AX_INSPECT_STAGE2_X, 0)) {
			...
			m_tInspect2Loop.Takt_Start(nTaktZone,52);
		}
		break;
	...
	case 70:	// Wait 회피 위치, Top1 Position 부터 진행한다.
		if (m_nInspect1Case < 80 && m_nInspect1Case > 7 && !g_objCommon.Check_Position(AX_INSPECT_STAGE1_X, 0)) {
			...
			if (dCurPos >= dT1Pos) {	//위치 확인
				m_nInspect2Case++; m_tInspect2Loop.Set_LoopTime(10000);
				m_tInspect2Loop.Takt_End(nTaktZone,52, TRUE);
			}
		}
		return TRUE;
	case 71:	// Stage Up
...
BOOL CSequenceMain::Inspection3_Run()
{
	...
	case 3:		// CM Align Master
		if (g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 1))
		{
			g_objLogFile.Save_PositionLog(gData.nPNoInspect[2],gData.nTNoInspect[2][0], -1, AX_MODULE_ALIGN_Z, MODULE_ALIGN_Z_StopDown);
			
			m_pDY06->oCmAlignMasterOpen = FALSE; m_pDY06->oCmAlignMasterClose = TRUE;
			g_objAJinAXL.Write_Output(6);
			m_nInspect3Case = -1; m_tInspect3Loop.Set_LoopTime(5000); 
	...
	case 6:		// Align Z Axis Move to Ready Up Position
		if (g_objCommon.Check_Position(AX_MODULE_ALIGN_Y, 0) &&
			m_pDX06->iCmAlignMasterOpen && !m_pDX06->iCmAlignMasterClose &&
			m_pDX06->iCmAlignSlaveOpen  && !m_pDX06->iCmAlignSlaveClose && g_objCommon.Get_InfoInspectVacOn(3) ) 
		{
	...
	case 7:		// 안전확인 & Move to Top1 Position	// Mirror 실린더 확인
		if (g_objCommon.Check_Position(AX_MODULE_ALIGN_Z, 0) && g_objCommon.Check_Position(AX_TOP1_MIRROR_Z, 0) && g_objCommon.Get_InfoInspectVacOn(3)) {
			...
			{	
				...
				else if (m_nInspect2Case > 17 && m_nInspect2Case < 50) 
				{	
					// Top2 ~ Unload 위치에 있을때
					double dCurPos = g_objAJinAXL.Get_Position(AX_INSPECT_STAGE2_X);
	...
	case 16:	// 다음 검사 위치 피치 이동
		...
		{			
			...
			else 
			{
				...
				{ 
					m_tInspect3Loop.Set_LoopTime(10000); 
					return TRUE;
				}
			}
		}
	...
	case 23:	// Top2 Shift X, Y Move
		...
		break;
	case 24:	// Send Move Complete
		if (g_objAJinAXL.Is_MoveDone(AX_INSPECT_STAGE3_X, dIs3PosX) && g_objCommon.Check_Position(AX_TOP2_SHIFT_Y, 1) 
			&& g_objAJinAXL.Is_Done(AX_VISION_TOP2_Z)) 
		{			
			g_objInspector.Set_SMoveComplete(INSPECTOR_PC3, "T2");
			m_nInspect3Case = 20; m_tInspect3Loop.Set_LoopTime(90000);	// 90초			
	...
	case 25:	// 다음 검사 위치 피치 이동
		...
		if (g_objAJinAXL.Is_MoveDone(AX_INSPECT_STAGE3_X, dIs3PosX) &&
			...
			if (nIs3ScanNo > gData.nBtmPickQt-1) 
			{
				// 모델별로 피치이동 횟수가 다르다.				
				nIs3ScanNo = 0;
				bTaktSave3 = FALSE;
	...
	case 52:	// Position Check
		if (g_objCommon.Check_Position(AX_INSPECT_STAGE3_X, 0)) {
			...
			m_tInspect3Loop.Takt_Start(nTaktZone, 52);
		}
		break;
	...
	case 70:	// Wait 회피 위치, Top1 Position 부터 진행한다.
		if (m_nInspect2Case < 80 && m_nInspect2Case > 7 && !g_objCommon.Check_Position(AX_INSPECT_STAGE2_X, 0)) {
			...
			if (dCurPos >= dT1Pos) {	//위치 확인
				m_nInspect3Case++; m_tInspect3Loop.Set_LoopTime(10000);
				m_tInspect3Loop.Takt_End(nTaktZone,52,TRUE);
			}
		}
		return TRUE;
	case 71:	// Stage Up
	...
	case 3:		// 정보전달(Stage->Picker), Btm2 Picker Grip Close
		if (g_objCommon.Get_Btm2PickerDown(0) && g_objCommon.Check_Position(AX_BTM2_PICKER_Z, 1))
		{
			...
			for (int i = 0; i < 10; i++) 
			{
				...
				if(gData.InfoBtm2Pick[i] > 0)
				{
					g_objLogFile.Save_PositionLog(gData.nPNoInspect[nB2pInspStageNo-1], gData.nTNoBtm2Pick[i], gData.nCNoBtm2Pick[i], AX_BTM2_PICKER_Z, BTM2_PICKER_Z_InspectDown);
				}
			}
	...
	case 23:	// 정보전달, Btm2 Picker Vac Off
		if (g_objCommon.Check_Position(AX_BTM2_PICKER_Z, 3))
		{
			for (int i = 0; i < gData.nBtmPickQt; i++)
			{
				...
				g_objLogFile.Save_PositionLog(gData.nPNoBtm2Pick, gData.nTNoBuffTray[nB2pWorkTray-1][nB2pTrayPosY][nBuffX+i], gData.nCNoBuffTray[nB2pWorkTray-1][nB2pTrayPosY][nBuffX+i], AX_BTM2_PICKER_Z, BTM2_PICKER_Z_BufferDown);
			}
...
BOOL CSequenceMain::BufferTray1_Run()
{
	...
	case 1:		// 안전확인.
		if (m_bUnloadLotEnd) {
			...
			m_tBuffTray1Loop.Takt_Start(nTaktZone, 0, TRUE);
	...
	case 52:	// Z Cylinder Up
		if (g_objCommon.Check_Position(AX_BUFFER_STAGE1_Y, 0)) {
			if (m_nBuffTray2Case >= 20 && m_nBuffTray2Case < 50) {
				...
				m_nBuffTray1Case = 80; m_tBuffTray1Loop.Set_LoopTime(10000);	// 후처리
			}
		}
		return TRUE;
	case 60:	// Lot End Check
		if (m_pDX09->iBufferStage1Up && !m_pDX09->iBufferStage1Down) {
			...
			m_tBuffTray1Loop.Takt_End(nTaktZone, 60, TRUE);
...
BOOL CSequenceMain::BufferTray2_Run()
{
	...
	case 1:		// 안전확인.
		if (m_bUnloadLotEnd) {
			...
			m_tBuffTray2Loop.Takt_Start(nTaktZone, 0, TRUE);
	...
	case 52:	// Z Cylinder Up
		if (g_objCommon.Check_Position(AX_BUFFER_STAGE2_Y, 0)) {
			if (m_nBuffTray1Case >= 20 && m_nBuffTray1Case < 50) {
				...
				m_nBuffTray2Case = 80; m_tBuffTray2Loop.Set_LoopTime(10000);	// 후처리
			}
		}
		return TRUE;
	case 60:	// Lot End Check
		if (m_pDX09->iBufferStage2Up && !m_pDX09->iBufferStage2Down) {
			...
			m_tBuffTray2Loop.Takt_End(nTaktZone, 60, TRUE);
...
BOOL CSequenceMain::SortPicker1_Run()
{
	...
	case 3:		// 정보전달, Picker Vac On
			...
			g_objCommon.Get_SortPicker1Down(0))
		{
			...
			for (int i = 0; i < gData.nSortPickQt; i++) 
			{
				...
				if(nSp1WorkBuff == 1) g_objLogFile.Save_PositionLog(gData.nPNoBuffTray[nSp1WorkBuff-1], gData.nTNoSortPick[0][i], gData.nCNoSortPick[0][i], AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Buffer1Down);
				if(nSp1WorkBuff == 2) g_objLogFile.Save_PositionLog(gData.nPNoBuffTray[nSp1WorkBuff-1], gData.nTNoSortPick[0][i], gData.nCNoSortPick[0][i], AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Buffer2Down);
	...
	case 5:		
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoSortPick[0], -1, -1, AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Ready );
	...
	case 15 :	// Picker Up
		if (g_objCommon.Get_SortPicker1OpenMulti(nSp1StartNo+1, nSp1DownSu)) 
		{
			...
			for (int i = 0; i < nSp1DownSu; i++) 
			{
				...
				g_objLogFile.Save_OutTray(sNgTray, gData.nTrayCntNG[gData.nPNoSortPick[0]-1][nSp1WorkNg], nSp1TrayPosX+i, nSp1TrayPosY, gData.nPNoSortPick[0], gData.nTNoSortPick[0][nSp1StartNo+i], gData.nCNoSortPick[0][nSp1StartNo+i]);
				g_objLogFile.Save_CmTrackingLog("NG", gData.nTrayCntNG[gData.nPNoSortPick[0]-1][nSp1WorkNg], nSp1TrayPosX+i, nSp1TrayPosY, gData.nPNoSortPick[0], gData.nTNoSortPick[0][nSp1StartNo+i], gData.nCNoSortPick[0][nSp1StartNo+i]);
	...
	case 16:	// Position Check
		...
		{			
			g_objLogFile.Save_PositionLog(gData.nPNoNgTray, -1, -1, AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Ready);
	...
	case 27:	// Position Check
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoGoodTray, -1, -1, AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Ready);
	...
	case 42:	// 정보전달, Vac Off	// Sort Picker = 0, NG Buffer = 1
		if (g_objCommon.Check_Position(AX_SORT_PICKER1_Z, 4) && g_objCommon.Get_SortPicker1DownMulti(nSp1StartNo+1, nSp1DownSu)) 
		{
			for (int i = 0; i < nSp1DownSu; i++) 
			{
				...
				g_objLogFile.Save_PositionLog(gData.nPNoSortPick[0], gData.nTNoNgBuffer[1][nSp1TrayPosX+i], gData.nCNoNgBuffer[1][nSp1TrayPosX+i], AX_SORT_PICKER1_Z, SORT_PICKER1_Z_NGBufferDown);
	...
	case 44:	// Position Check
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoNgBuffer[1], gData.nTNoNgBuffer[1][0], -1, AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Ready);
	...
	case 54:
		if (g_objCommon.Get_InfoSortPicker1Close() && g_objCommon.Get_InfoSortPicker1Check() &&
			g_objCommon.Check_Position(AX_SORT_PICKER1_Z, 0) && g_objCommon.Get_SortPicker1Up(0))
		{
			g_objLogFile.Save_PositionLog(gData.nPNoSortPick[0], gData.nTNoSortPick[0][0], -1, AX_SORT_PICKER1_Z, SORT_PICKER1_Z_Ready);
			g_objCommon.Move_Position(AX_SORT_PICKER1_P, 0);	// Tray Pitch
			m_nSortPick1Case = 7; m_tSortPick1Loop.Set_LoopTime(5000);
...
BOOL CSequenceMain::SortPicker2_Run()
{
	...
	case 3:		// 정보전달, Picker Vac On
			...
			g_objCommon.Get_SortPicker2Down(0))
		{
			...
			for (int i = 0; i < gData.nSortPickQt; i++) 
			{
				...
				if(nSp2WorkBuff == 1) g_objLogFile.Save_PositionLog(gData.nPNoBuffTray[nSp2WorkBuff-1], gData.nTNoSortPick[1][i], gData.nCNoSortPick[1][i], AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Buffer1Down);
				if(nSp2WorkBuff == 2) g_objLogFile.Save_PositionLog(gData.nPNoBuffTray[nSp2WorkBuff-1], gData.nTNoSortPick[1][i], gData.nCNoSortPick[1][i], AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Buffer2Down);
	...
	case 5:
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoSortPick[1], -1, -1, AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Ready );
	...
	case 15 :	// Picker Up
		if (g_objCommon.Get_SortPicker2OpenMulti(nSp2StartNo+1, nSp2DownSu)) 
		{
			...
			for (int i = 0; i < nSp2DownSu; i++) {
				...
				g_objLogFile.Save_OutTray(sNgTray, gData.nTrayCntNG[gData.nPNoSortPick[1]-1][nSp2WorkNg], nSp2TrayPosX+i, nSp2TrayPosY, gData.nPNoSortPick[1], gData.nTNoSortPick[1][nSp2StartNo+i], gData.nCNoSortPick[1][nSp2StartNo+i]);
	...
	case 16:	// Position Check
		...
		{	
			g_objLogFile.Save_PositionLog(gData.nPNoNgTray, -1, -1, AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Ready);		
			if (Check_NgTrayFull()) {
				if (m_nNgTrayCase == 10) m_nNgTrayCase = 11;
			}
	...
	case 27:	// Position Check
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoGoodTray, -1, -1, AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Ready);
			// 한Case에서 오는게 아니라 따로 추출해서 입력해준다.
			if (Check_SortPickerEmpty(2)){
	...
	case 42:	// 정보전달, Vac Off	// Sort Picker = 0, NG Buffer = 1
		if (g_objCommon.Check_Position(AX_SORT_PICKER2_Z, 4) && g_objCommon.Get_SortPicker2DownMulti(nSp2StartNo+1, nSp2DownSu)) {
			for (int i = 0; i < nSp2DownSu; i++) {
				...
				g_objLogFile.Save_PositionLog(gData.nPNoSortPick[1], gData.nTNoNgBuffer[0][nSp2TrayPosX+i], gData.nCNoNgBuffer[0][nSp2TrayPosX+i], AX_SORT_PICKER2_Z, SORT_PICKER2_Z_NGBufferDown);
	...
	case 44:	// Positon Check
		...
		{
			g_objLogFile.Save_PositionLog(gData.nPNoNgBuffer[0], gData.nTNoNgBuffer[0][0], -1, AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Ready);
	...
	case 54:
		if (g_objCommon.Get_InfoSortPicker2Close() && g_objCommon.Get_InfoSortPicker2Check() &&
			g_objCommon.Check_Position(AX_SORT_PICKER2_Z, 0) && g_objCommon.Get_SortPicker2Up(0))
		{
			g_objLogFile.Save_PositionLog(gData.nPNoSortPick[1], gData.nTNoSortPick[1][0], -1, AX_SORT_PICKER2_Z, SORT_PICKER2_Z_Ready);
			g_objCommon.Move_Position(AX_SORT_PICKER2_P, 0);	// Tray Pitch
			m_nSortPick2Case = 7; m_tSortPick2Loop.Set_LoopTime(5000);

			m_tSortPick2Loop.Takt_End(nTaktZone, 53);
			m_tSortPick2Loop.Takt_Start(nTaktZone, 54);
			m_tSortPick2Loop.Takt_End(nTaktZone, 54, TRUE);
		}
		break;
...
BOOL CSequenceMain::EmptyTrayX_Run()
{
	...
	case 8:		// Move to Wait Position
		if (g_objCommon.Get_EmptyTrayXUp() && m_pDX13->iEmptyTrans1Exist) {
			...
			m_nEmptyTrayXCase++; m_tEmptyTrayXLoop.Set_LoopTime(15000);
		}
		break;
	...
	case 14:	// Angle Port 진행.
		if (g_objAJinAXL.Is_Done(AX_EMPTY_PORT_Z) && g_objCommon.Get_EmptyTrayYUp() && m_pDX13->iEmptyTrans2Exist) {
			if (m_nEmptyTrayElCase == 10) 
			{
				...
				m_nEmptyTrayYCase++; m_tEmptyTrayYLoop.Set_LoopTime(15000);
			}			
		}
		break;
	case 15:	//  안전 확인

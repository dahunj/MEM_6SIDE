...
void CLogFile::Save_TerminalLog(const CString& sLog)
{
	...
	CString strPath = gData.sLogPath + "\\Terminal";
...
void CLogFile::Save_ECMTracking(const CString& sLog, int nTrayCount, int nPosX, int nPosY, int nPortNo, int nTrayNo, int nCmNo)
{
	...
	if (gLot.sLotID[nPx] == "") gLot.sLotID[nPx] = "LOT_ID";
	strFile.Format("%s\\%s_%04d%02d%02d%02d_AVITracking.csv", strPath, gLot.sLotID[nPortNo-1], time.wYear, time.wMonth, time.wDay, time.wHour);
...
void CLogFile::Save_PositionLog(int nPNo, int nTNo, int nCmNo, int nAxis, int nMoveIdx)
{
	...
	CString sName = "";
	g_objLogFile.Get_PosName(nAxis, nMoveIdx, sName);
...
void CLogFile::Get_PosName(int nAxis, int nMoveIdx, CString &sPosName)
{
